...
TOOL_SPEC = {
    "name": "sketcher.split_geometry",
    "description": "Split one native Sketcher curve at the picked sketch-space point, equivalent to using the Sketcher split tool.",
    "contextual": True,
    "parameters": {
        "type": "object",
        "properties": {
...
def run(
    ...
    def _split() -> dict[str, Any]:
        ...
        target.split(index, App.Vector(float(x), float(y), 0.0))
    ...
    return active_response(service, sketch, run_freecad_transaction("Split Sketcher geometry", _split))
